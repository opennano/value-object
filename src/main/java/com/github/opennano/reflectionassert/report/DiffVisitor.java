...
/**
 * This class visits the tree of {@link Diff} objects produced by a {@link ComparerManager}. While
 * visiting each diff the full property path to that diff is provided, so that a flattened list of
 * diffs can be provided to the end user. Also provided is a {@link DiffView} used by the visited
 * diffs to aggregate text results suitable for display to the end user.
 *
